...
--member테이블의 구조확인
DESC member;
--각종 제약조건은 USER_CONSTRAINTS 테이블에 자동으로 저장된다.
DESC USER_CONSTRAINTS;
SELECT  CONSTRAINT_NAME FROM USER_CONSTRAINTS;
SELECT  CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE (TABLE_NAME='member'); --테이블이름은 대문자로 저장된다. 소문자로 테이블만들어도 
SELECT  CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE (TABLE_NAME='MEMBER'); --DB에는 대문자로 저장됨
SELECT  CONSTRAINT_NAME FROM USER_CONSTRAINTS WHERE (TABLE_NAME=UPPER('member'));   --DBA_user ,탭 ,USER_CONSTRAINTS; 메타데이터                                                             
