...
public class SimpleDAO {
	...
	//3.updateSimple()
	public void updateSimple(SimplieDTO sDTO) {
		try {
			conn=ds.getConnection();
	...
	//5. dto를 받는다오 insertSimple()
	public void insertSimple(SimplieDTO sDTO) {
		try {
			conn = ds.getConnection();
			sql="INSERT INTO SIMPLE (BNO, BWRITER,BTITLE,BCONTENT,BDATE) VALUES(SIMPLE_SEQ.NEXTVAL,?,?,?,SYSDATE)";
			ps =conn.prepareStatement(sql);
			ps.setString(1, sDTO.getbWriter());
			ps.setString(2, sDTO.getbTitle());
			ps.setString(3, sDTO.getbContent());
			ps.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
