...
public class ImageBoardSingleUploadCommand implements ImageBoardCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		//동일한 이름을 가진 파일이 업로드 되지않도록 직접 파일이름을 수정해서 올린다.
		...
		try {
			...
			//2)업로드
			//2-1)파일이 저장될 서버내 (/resources/storage) 경로를 알아낸다.
			String realPath = mr.getSession().getServletContext().getRealPath("/resources/storage");
			//2-2/resources/storage 경로가 존재하지않으면 필요한 경로를 만든다.
			//new File(경로): 경로로 디렉토리만 사용되면 디렉토리로 인식한다.
			File directory = new File(realPath);
			if(!directory.exists() ) {
				directory.mkdirs(); //mkdirs하위 디렉토리를 모두만든다.
			}
